...
public static class ConfigurationManager
{
    public static AppSettings LoadConfiguration()
    {
        var settings = new AppSettings
        {
            ...
            NoticeServiceAllowIpList = GetAppSetting("NoticeServiceAllowIpList", "192.168.100.4").Split(',', StringSplitOptions.RemoveEmptyEntries),
            AuthIp = GetAppSetting("AuthIp", "192.168.100.4"),
            AuthPort = GetAppSetting("AuthPort", 2108),
            AuthConnCount = GetAppSetting("AuthConnCount", 3),
            LogDirectory = GetAppSetting("LogDirectory", "logs"),
